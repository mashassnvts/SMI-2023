...
namespace MFST
{
	...
	bool Mfst::start(Log::LOG log)
	{
		...
		case LENTA_END:
		{
			...
			cout << "\tСинтаксический анализ выполнен без ошибок." << endl;
			*log.stream << setfill('=') << setw(89) << '=' << endl;
			*log.stream << "\tСинтаксический анализ выполнен без ошибок." << endl;
			*log.stream << setfill('=') << std::setw(90) << '=' << endl;
			rc = true;
			break;
		}
	...
	char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "Ошибка %d: строка %d,	%s", err.id, lex.table[lpos].line, err.message);
			rc = buf;
		}
		return rc;
	}
